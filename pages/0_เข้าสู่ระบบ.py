...
from auth import login, is_authed  # จาก auth.py เดิมของคุณ

st.set_page_config(
    page_title="เข้าสู่ระบบ MedEquip Pro Lab",
    page_icon="🩺",
    layout="wide",
    initial_sidebar_state="collapsed",
)

# ถ้าล็อกอินอยู่แล้ว → ส่งไปหน้า Dashboard เลย
if is_authed():
...
st.markdown(
    ...
    .stApp {
        background: radial-gradient(circle at top, #C7D2FE 0%, #6366F1 35%, #4C1D95 80%);
    }
...
with st.form("login_form"):
    st.markdown(
        """
        <div class="login-title-main">MedEquip Pro Lab</div>
        <div class="login-title-sub">Medical Equipment & Quality Management System</div>
